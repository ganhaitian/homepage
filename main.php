...
<html dir="ltr" lang="en-US">
			...
			<div class="container main clearfix">
				...
				<aside id="sidebar">
					...
					<div id="categories-2" class="sidebar_box widget_categories clearfix">
						...
						<ul>
						<?php
							$result=mysql_query("select * from g_homepage_type where pk_menu=$current_menu");
							while($row=mysql_fetch_array($result)){
								echo "<li class='cat-item cat-item-$row[pk_type]' >";
								echo "<a href='http://welovenature.hostoi.com/category/bungling/' title='View all posts filed under $row[type_name]'>$row[type_name]</a>";
								echo "</li>";
							}
						?>
						</ul>
					...
					<div id="recent-posts-3" class="sidebar_box widget_recent_entries clearfix">		
						...
						<ul>
						<?php
							$result=mysql_query("select * from g_homepage_article order by ts desc limit 0,6");
							while($row=mysql_fetch_array($result)){
								echo "<li>";
								echo "<a href='$row[article_url]' title='$row[article_title]'>$row[article_title]</a>";
								echo "</li>";
							}
						?>
						</ul>
	...
	<!-- .container -->
			...
			<div class="container clearfix" >
				...
				<div id="text-3" class="footer_box widget_text clearfix">
					<h4>Info</h4>			
					<div class="textwidget">
						<p>Oh, the great and lovely white rabbit, how he flies through the skies as any old cat does. The flowers smell so good when the rain pours over the hills and the rabbits eat meat with the chickens. The great thing about flying is that anybody can do it, well anybody excepting the flowers.</p>
	</div>
